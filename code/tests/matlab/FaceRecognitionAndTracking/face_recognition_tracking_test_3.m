...
videoFileReader = vision.VideoFileReader('..\..\media\PeopleVideo\face_video_3.mp4');
...
v = VideoWriter('..\..\media\test_video_4.avi');
...
while ~isDone(videoFileReader)
    ...
    if size(visiblePoints, 1) >= 2 % need at least 2 points
        ...
        videoFrame = insertText(videoFrame,[100 100],'detected');
        ...
        %imshow(label_detected);%show label
    
    else    
        videoFrame = insertText(videoFrame,[100 200],'not detected');
    ...
    writeVideo(v,videoFrame);%save frame to video file
